...
def main():
  ...
  completion = client.chat.completions.create(
    ...
    messages=[
      ...
      {"role": "user", "content": f"Create a table with the following information: The title of the table is 'American Presidents Birthday Briefing'. The subtitle is 'Updated as of {current_month_name} {current_year}'. The column headers for the table should be 'Name', 'Birthday', and 'Fun Fact'. For each president born in the month of {current_month_name} the table will have a row corresponding to that president's name, birthday, and a fun fact. The fun fact is a short and amusing anecdote about the president. Important Note: Include all the presidents born in {current_month_name}. Do not include any presidents born in any other months. The whole table should be written as a file in the markdown format. Just provide the markdown, no introduction, no comments, no notes, no code fencing, nothing outside the markdown."}
    ]
  )
